...
function requireApiKey(credentials: ResearchCredentials): string {
  if (credentials.exaApiKey === undefined) {
    throw new MissingCredentialError('exaApiKey', 'required for the Exa search lane');
  }
  return credentials.exaApiKey;
}
...
export function createExaProvider(credentials: ResearchCredentials): SearchProvider {
  return {
    name: 'exa',

    enabled: credentials.exaApiKey !== undefined,

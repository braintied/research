...
export function createCrawl4aiProvider(credentials: ResearchCredentials): SearchProvider {
  return {
    name: 'crawl4ai',
    ...
    capabilities: {
      ...
      fetch: true,
      extract: true,
      backends: ['crawl4ai', 'direct_fetch'],
    },

    // No API key of its own: direct fetch always works, and the browser crawler
    // engages only when the host configured an endpoint + reviewed allowlist.
    enabled: true,
    ...
    async fetch(url: string, _signal?: AbortSignal): Promise<FetchResult> {
      ...
      try {
        markdown = await crawlWithCrawl4AI(credentials, url);
      } catch (err) {
      ...
      if (markdown === null || markdown.trim().length === 0) {
        const result = FetchResultSchema.parse({
          ...
          url,
          fetch_status: 'failed',
          fetch_error: 'crawlWithCrawl4AI and directFetchAsText both returned no content',
        });
        return result;
      }
